...
class Client():
    ...
    def __sendMessage__(self): #发送信息
        ...
        Message = Message + '|' +str(self.LogStatus)
    ...
    def __listening__(self,c):
        ...
        while self.Serstatus:
            ...
            try:
                ...
                pass
            if RecMessage !='':
                RecMessage, Userstatus = RecMessage.split('|')
                if '$' in RecMessage:
                    if RecMessage in self.data:
                        self.__send__(c,'登陆成功！')
                    else:
                        self.__send__(c,'登陆失败！')
                if Userstatus == '1':
